...
## V_R9 — 2026-05-02 MT redistribute 3-direction parallel validation (closed)

**Problem**: 用戶 audit MT 輸出仍然有：
1. ZH 中切問題（mechanical char-proportion redistribute 切 X·Y name 中間）
2. Single-char/short ZH segments（「找。」、「了」 等 orphan）
3. Empty ZH segments（lopsided proportion 把 short EN seg 分配到 0 chars）

**Method**: 並行 3 個 agent，每個 5-round 自我迭代驗證一個方向。

### Direction 比對結果（Real Madrid 82-seg baseline + 30-seg slice for B/C）

| | Baseline (Hybrid V2) | **A** (redistribute v2) ✅ | **B** (LLM markers) ⚠️ | **C** (per-seg translate) ❌ |
|---|---|---|---|---|
| single_char% | 3.66% | **0%** | 0% (R2 batch=5) | 3.3% |
| mid_cut% | 4.88% | **3.66%** | 6.9% | 30.0% ❌ |
| wrap_hc% (NTF General) | 3.66% | **2.44%** | 6.7% ❌ | 0% |
| name_split | 0 | **0** | 0 | 0 |
| empty_seg | 3 | **0** ✅ | n/a | n/a |
| API calls (30 segs) | ~5 | **0** | 6 | 30 (6×) |
| Marker compliance | n/a | n/a | 83% (17% fallback) | n/a |
| Implementation | — | 簡單 (定 ~150 行) | 中（要 OpenRouter API）| 複雜（per-seg + cost）|

### B reject reason

LLM marker compliance 只 83%（batch=5）→ 17% batch 退回 char-proportion fallback (= baseline 行為)。LLM 只決定切位，唔解 char budget overflow → wrap_hc 6.7% 比 A 嘅 2.44% **差 3 倍**。

### C reject reason

- mid_cut% 30% 比 baseline 差 **30pp**：EN ASR 喺 pause 切、唔係 clause 切，per-seg 翻譯如實反映 mid-clause cut → ZH 用「……」trailing
- Context window REGRESSES (C2/C4 比 C1 差)
- 短 fragment 有 hallucination（"Tifo football by the Athletic." → "Athletic Bilbao"）
- 6× API cost、~7× latency

### A 落地 production

`backend/translation/sentence_pipeline.py` 加：
1. **`_build_locked_mask(text)`**: 禁切位 — middle-dot 名 (X·Y), number+量詞 run, bracket-adjacent
2. **`_conjunction_bonus(text, p)`**: +20/+30 for clause-conjunction continuation
3. **`_find_break_point` 接受 `locked` + `use_conjunction_bonus`** kwargs
4. **`redistribute_to_segments` 加 lopsided rebalance** (`min_chars_per_segment=4`, `lopsided_threshold=0.30`)
5. **`_orphan_merge`** safety net (default OFF — opt-in via `enable_orphan_merge=True`)

### A 落地 production 量度（in-place 改動，重跑 baseline 82-seg）

| Metric | Baseline | Production A | Δ |
|---|---|---|---|
| Empty segments | 3 | **0** | -3 ✅ |
| Single-char (≤2c) | 2 | **0** | -2 ✅ |
| Mid-cut | 3 | 3 | (持平) |
| Wrap hard-cut (NTF General) | 3 (3.7%) | **2 (2.4%)** | -1 |
| Segs changed from baseline | — | 16 (19.5%) | — |

✅ 同 agent prototype R2 best 完全一致。

### 5-sample 4-way audit (Real Madrid 82-seg)

- **#26**: BASE「」(empty) → A「在中場方面，」(6c) ✅
- **#35**: BASE「」(empty) → A「從紙面陣容來看，」(7c) ✅
- **#61**: BASE「」(empty) → A「羅德里戈」(4c name, borderline 4-char floor) ⚠️
- **#65-66**: BASE「...中尋」+「找。」(syllable break, 2c orphan) → A「...離隊，」+「皇馬的替補人選或許可從現有陣容中尋找。」✅
- **#72-73**: BASE「...的空檔」+「。」(period orphan) → A「...遊走」+「的空檔。」✅

### 5 新 pytest

- test_redistribute_lopsided_rebalance_fills_empty_seg
- test_redistribute_locked_mask_prevents_name_split
- test_redistribute_locked_mask_number_run
- test_redistribute_orphan_merge_disabled_by_default
- test_redistribute_conjunction_bonus_prefers_clause_break

### 仍未驗證嘅 caveat

- Validation 喺 baseline 82-seg corpus 上做，唔係 α-shipped 122-seg corpus（agent 用咗 22-raw alpha output 唔係實際 122-seg 跑出嚟嘅 α 結果）。下一步可揀其他 cached translation 做 cross-corpus 驗證。
- B/C 只 cover 頭 30 段；長片 corpus 未 close。
