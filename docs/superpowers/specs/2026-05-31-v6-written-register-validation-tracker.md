...
## 整合驗證（live end-to-end，2026-05-31）✅ PASS
**設置**：新 pipeline `1443afcb`「[v6] 賽馬廣播 (書面語)」設為 active → 重新上傳真片（賽後兩點晚，283 MB）→ 完整 `_run_v6`（VAD→Qwen3→mlx→merge→**2-refiner chain**）跑足 **560s**（喺 `R5_QWEN3_TIMEOUT_SEC=900` 內）→ 新 file `e4eda91bb304`，337 段，`status=done` 無 error。**`stage_outputs` 有 6 個 key（0-5）** —— 比口語 pipeline 多一個 stage，證實第二個 refiner 真係執行咗。

| 指標 | 結果 | 門檻 | 判定 |
|---|---|---|---|
| Source 口語 markers/100（raw qwen3 merged 輸入）| 13.32 | — | baseline |
| **Output 書面語 markers/100** | **0.07** | ≤2.0 | ✅✅ |
| over-cap >24 字 | 5/337（1.5%）| 監察 | ✅（低過口語 1.8% baseline）|
| empty zh | 0/337 | — | ✅ |
| 阿拉伯數字保留 | `135` → 「負重 135 磅」 | 保留 | ✅ |

**質性**：raw qwen3 filler（「準備起步啦咁啊出閘嘅時候咧咁啊都望住咧咁啊喺內欄啊」）經 pass-1 口語 refiner + pass-2 書面語 refiner 後變「準備起步。出閘時均望向內欄。」；馬名（飛輪八／光年情長／加州勇士／「航至尊」／鑽石）全保留、register 完全轉換。讀落係乾淨現代正式書面語。比 prototype 更強證據（測試完整 chain on raw 輸入，唔止 pass-2 on clean）。

**口語 pipeline regression**：`4696bbaa` config **byte-identical**（git collateral check：4 個 commit 只動 4 個新檔，舊 pipeline/refiner/app.py 零改動）+ regression test `test_colloquial_pipeline_unchanged_single_refiner` PASS（chain 仍 1 元素）→ 零 regression（config 隔離保證；唔重跑 9 分鐘 V6 以省成本，由 config-isolation + test 擔保）。

**結論**：✅ 端到端整合通過。書面語 pipeline 已可生產使用。

## 下一步
User review 本 tracker → Superpowers brainstorm → spec → plan → 落 config（cherry-pick prior art：prompt template + refiner profile + pipeline JSON，`user_id`→null，鬆 test assertion，prompt 按上面 dial 調成正式書面語）→ 整合 re-run 真片確認端到端。**[全部完成 2026-05-31]**
