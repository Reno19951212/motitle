...
| 🎥 **實時直播** | 從攝像頭或屏幕共享捕捉，支援分段模式和串流模式（低延遲） |
...
### v1.7 — 串流模式（whisper-streaming 整合）
- **新增串流模式**：Live 頁面可切換「分段模式」（每 3 秒批次轉錄）和「串流模式」（連續 PCM 音訊串流，更低延遲）
- **whisper-streaming 整合**：使用 LocalAgreement-2 策略，兩次連續轉錄結��一致才確認輸出
- **連續音訊擷取**：前端以 ScriptProcessorNode 擷取 16kHz 原始 PCM，每 ~256ms 傳送一次
- **自動偵測**：啟動���檢查 `whisper-streaming` 是否可用，不可用時自動隱藏串流選項
- **可選安裝**：`whisper-streaming` 為可選依賴，不影響原���分段模式功能
- 新增 API：`GET /api/streaming/available`
