...
**重新處理：** 檔案完成後，檔案卡上嘅「🔄 重新處理」按鈕可以將成條片重新行一次 pipeline — 會彈返上載時嘅語言設定視窗（已預填現有設定），你可以改輸出語言、翻譯風格、詞彙表之後再開始。注意：重新處理會清掉現有字幕、譯文同批核狀態。
...
右側段落詳情面板中，每個語言欄（第一語言／第二語言）標籤右邊有一個「**✦ AI**」按鈕：

1. 點擊後彈出「AI 輔助修改」視窗，顯示該段嘅現有字幕（修改前）
2. 可以直接輸入修改指令（例如「將『傷感』改做『難過』」），或點擊快速選項自動填入指令（可再修改）：
   - **⇄ 對照翻譯** — 根據另一語言嘅意思重新翻譯呢段字幕
   - **改更書面** — 將語氣改得更書面正式
   - **改更口語** — 將語氣改得更口語自然
   - **精簡句子** — 喺唔改變意思嘅前提下精簡句子
3. 按「生成」後 AI 會輸出「修改後」預覽（一般需時數秒）
4. 滿意就按「**套用**」寫入並自動批核該段；唔滿意可以「再生成」或修改指令重試；「取消」／Esc 直接關閉，唔會改動任何字幕

> AI 只會修改你撳掣嗰一個語言欄；另一語言會作為參考上下文一齊俾 AI。所有修改都係先預覽後套用 — AI 唔會未經你確認就改動字幕。

**AI Rerun 重跑（僅限輸出語言檔案）：**

如果某段嘅轉錄或翻譯成個唔對，可以叫 AI 由音訊開始重做：

- **單段**：右側段落詳情頂部（「✓ 已批核」左邊）撳「**⟳ AI Rerun**」— 系統會重新截取該段音訊、重新語音轉錄，再重新生成所有輸出語言嘅字幕
- **批量**：段落表頂部撳「**⟳ Rerun 未批核 (N)**」— 將全部未批核段落逐段重跑，有進度顯示（x/y），可隨時「取消」（已完成嘅段保留）
- 重跑結果**直接覆蓋**原有字幕，並將段落重設為「未批核」俾你再審核
- 重跑期間唔可以做段落分割／合併／渲染（會提示等完成）

> 提示：時間好短（一秒以下）嘅段落，重跑準確度會下降 — 建議先用「合併下一段」將段落整返自然長度再重跑。

**段落表顏色：** 已批核嘅段落成行字幕會以**綠色**顯示，一眼分清邊啲段已完成、邊啲仲未審。

**中文書面語質量提升（賽馬內容）：**

輸出中文書面語時，系統會參考前後句嘅上下文嚟改寫，唔再逐句孤立處理。賽馬位置術語（如「尾二」＝倒數第二、「尾三」＝倒數第三）會正確譯成「倒數第N」而唔會誤解成「第N位」；詞彙表入面嘅馬名會逐字保留，唔會被當普通詞改走（例如「好友心得」唔會變成「獲好評」）。系統亦會避免補充原文冇嘅內容（防止無中生有嘅評語或賽果）。

**賽馬行話用詞（2026-07-09 新增）：**

用「賽馬」風格翻譯英文賽馬廣播時，系統識得多啲廣播行話，唔再照字面直譯：`track work / gallops` 譯「晨操」（唔係「工作/該場地的表現」）、`back in the field / closer` 譯「後上」、`newcomer` 譯「初次上陣（新馬）」（唔係「新人」）、`out of a X mare` 嘅 X 當父系馬名保留（如 `Reset` 唔會譯成「重置」）；距離單位一律用「米」（唔用「公尺」）；騎師 Luke Ferraris 譯「霍宏聲」。呢批係參考香港賽馬會專業廣播字幕、逐句對照 MoTitle 輸出後補強，只喺「賽馬」風格生效，通用/體育新聞風格不受影響。

**確定統一（唔靠 AI 記性，2026-07-09 新增）：** 有啲嘢一定要每次一樣，唔應該靠 AI 每次自己記 —— 所以系統喺翻譯之後加咗一個**確定性校正步驟**：距離單位「公尺」一律改「米」（所有中文輸出）；賽馬騎師名一律統一做香港賽馬會正名（例如就算 AI 保留咗英文「Luke」或者音譯錯「盧克」，都會校正做「霍宏聲」）。呢個係機械替換、每次一致，唔會再「時中時唔中」。校正記錄會喺校對頁「詞彙對照」顯示（標「單位正規化」/「騎師正名」）。

**粵語同音字自動糾正（馬名／術語）：**

廣東話影片揀咗詞彙表處理時，系統會喺轉錄之後自動用**粵語讀音**對照詞彙表，修正同音錯字 — 例如「升制快車」會自動改返做「星際快車」、「內藍米字」改返做「內欄位置」。修正會記錄喺校對頁嘅詞彙對照，標示「語音糾正」，可以逐個檢視同還原。讀音完全相同嘅直接修正；讀音接近嘅會交 AI 按上下文判斷先至改，唔會亂改正常字句。

**英文馬名自動統一寫法（2026-07-07 新增）：**

英語影片揀咗詞彙表處理時，字幕入面嘅英文馬名／騎師名會**自動統一成詞彙表嘅原樣寫法** — 例如詞彙表存全大寫 `GOLDEN SIXTY`，字幕出咗 `golden  sixty`（就算中間多咗空格、大小寫唔一致）都會自動改返做 `GOLDEN SIXTY`。中文字幕軌亦因此更穩定咁對中詞彙表嘅中文馬名。同粵語糾錯一樣分兩層：寫法完全對得上嘅直接修正（標示「英文糾正」）；ASR 聽錯嘅近音字（例如 `Only You` 其實係馬名 `ONLY U`）會交 AI 按上下文判斷先至改（標示「英文糾正(AI判決)」）。全部修正記錄喺校對頁詞彙對照，可逐個檢視。呢個修正喺**上傳／重新處理**時進行；自 2026-07-15 起，「**全部重新生成**」同單段 **AI Rerun** 都會重跑呢層 base 糾錯（英文源同粵語源都係），所以加咗近音別名之後，喺舊檔撳「全部重新生成」就會即時生效。

**術語表宣告別名（近音別名，2026-07-15 新增）：**

有啲專名 ASR 成日聽錯成同一個固定嘅錯樣（例如馬名 `SPEEDY SMARTIE` 常被聽成 `Speedy Smarty`、`好友心得` 常被聽成 `好有心得`）。以前呢類近音錯要靠 AI「時中時唔中」咁估返；而家你可以喺術語表**直接宣告**「聽錯樣 → 正名」嘅對應，系統會**每次確定性咁**改返做正名，零 AI、零估。

- **點填**：喺術語表每個詞條，除咗原本嘅「原文（source）」同「譯名（target）」，可以額外填：
  - **近音別名（source_variants）** — 英文源專用：列出 ASR 常聽錯嘅英文寫法（例如 `Speedy Smarty`、`Speedy Smart`），系統會喺英文字幕軌自動改返做原文正名。
  - **譯名別名（target_aliases）** — 中文譯名嘅常見聽錯／打錯樣（例如 `好有心得`），系統會喺中文字幕軌自動改返做正確譯名。
- **安全閘（唔會亂改正常句）**：中文別名**至少 3 個字**先至生效（`電流`、`尾指` 呢類 2 字太危險，一律唔收，防止改爛正常字句）；英文別名以完整字界比對（`ACE` 唔會咬入 `RACE`）。宣告別名同正名一樣嘅話會自動略過。
- **CSV 匯入／匯出（4 欄）**：術語表 CSV 而家係四欄 —
  ...
  同一欄多個別名用**分號 `;`** 分隔。**向後兼容**：舊有嘅 2 欄（`source,target`）同 3 欄（`source,target,target_aliases`）CSV 照樣匯入得到；匯出一律出 4 欄。
- **對舊檔生效**：加咗別名之後，喺該檔案撳「**全部重新生成**」或者對個別段落做「**AI Rerun**」，系統會補跑一次糾錯層，令新別名對已處理嘅舊檔真正生效（重複行係安全嘅，唔會越改越亂）。改動會記錄喺校對頁「詞彙對照」，標示「宣告別名」。

**一鍵閉環（唔使死記 CSV，2026-07-15 新增）：**

除咗 CSV，而家你可以喺介面上一路睇一路㩒，令聽錯直接變永久別名：

- **術語表頁直接填**：喺**術語表頁**（Glossary.html）揀一條詞條，詳情面板嘅「原文」下面多咗「**近音寫法（原文別名）**」一列 chip — 撳「＋ 加入」打個聽錯樣（例如 `Speedy Smarty`）即刻存，撳 chip 上嘅「×」移除；詞條列表該行會顯示「近音 N」小標籤，搜尋框亦搵得到別名。呢個等同直接編輯 `source_variants`／`target_aliases`，唔使開 CSV。
- **校對頁一鍵回饋**：喺**校對頁**撳「🔍 掃描詞彙表」，掃描視窗除咗「待修正／已符合」，仲多咗一節「**疑似聽錯**」— 系統用確定性近音比對（**零 AI**）揪出字幕入面同術語表／行話表好接近但唔完全一樣嘅寫法（例如 `Malpenza ≈ MALPENSA`）。**疑似聽錯係 opt-in**：要撳掃描視窗入面嘅「**🔎 搵疑似聽錯**」掣先會掃描出現（近音掃描喺大檔會較慢，每條語言軌最多掃 400 段；預設嘅精確掃描保持毫秒級）。逐項有「**＋ 加為近音別名**」掣，撳一下就把嗰個聽錯樣寫入對應詞條嘅別名（英文近音 → `source_variants`、中文譯名近音 → `target_aliases`、行話 → 系統行話表）。撳完之後同樣要「**全部重新生成**」先對成個舊檔生效。
- **已宣告別名一目了然**：掃描視窗有獨立一節「**已宣告別名**」，列出字幕入面命中你已宣告嘅近音別名但仲未改嘅位置，視窗頂部亦有 banner 提醒（「✍ 有 N 處字幕命中你宣告嘅近音別名」）— 即係話宣告咗但仲未生效。撳「**全部重新生成**」先會真正改寫呢啲位置。
- **加別名有安全提示**：無論喺術語表頁、校對頁詞條編輯視窗定掃描視窗一鍵加，系統都會即時檢查你填嘅別名 — 太短唔會生效（英文折疊後太短／中文少過 3 字）或者係常用英文詞（可能誤中日常字句）會彈出**非阻斷警告**；警告只提示、唔會阻止儲存（宣告別名係你明文承擔嘅逃生門）。
- **系統行話表（人人可睇、管理員先改得）**：賽馬等專業行話（如「殿後」「後上」）由一份全局共用嘅**系統行話表**管理。所有登入用戶喺術語表頁都會見到「**系統行話表**」入口；管理員可以為每個行話術語加近音聽錯樣（例如「殿後」加「電流位」），非管理員開到只讀視圖（睇得到內容，但冇修改掣）。因為係全局資源，修改只限管理員。

**校對頁詞彙表面板升級（2026-07-16 新增）：**

校對頁左側詞彙表 panel 唔再係淨睇 — 而家可以直接搜尋同編輯詞條：

- **搜尋**：panel 頂部搜尋框（🔍 搜尋原文/譯文/近音…）即時過濾詞條，原文、譯文、近音寫法、譯文別名都搵得到。
- **編輯／新增**：每條詞條有「✎」掣，panel 有「+ 新增」掣 — 兩個都開同一個編輯視窗，可以改原文／譯文，仲可以直接加減「**近音寫法（原文別名）**」同「**替代寫法（譯文別名）**」chip，唔使離開校對頁去術語表頁。
- **「·近N」標記**：詞條列表有宣告咗近音別名嘅條目會顯示「·近N」小標籤（N = 別名數量），一眼睇到邊啲詞條已有宣告。

> 提示：「近音寫法（原文別名）」只對**英文源**影片生效（改寫英文字幕軌）；中文（粵語）聽錯請填「**替代寫法（譯文別名）**」（改寫中文字幕軌）。

**尋找與取代（⌘F）：**

校對頁按 `⌘F`（Windows `Ctrl+F`）開「尋找與取代」視窗：輸入字詞即時列出全部語言欄的匹配段落（可剔「只搜未批核」）。點選任何一行會跳到該段並預覽影片。每行有三個動作：

- **取代** — 修改文字，批核狀態保持不變
- **取代並批核** — 修改文字並直接標為已批核
- **略過** — 跳過此行（可隨時還原）

右上「全部取代」可一次過處理所有剩餘行（批核狀態保持不變）。視窗不會阻擋頁面操作，可拖動移位；`Esc` 關閉，重開會保留上次查詢。

**AI 助手（僅限輸出語言檔案，NEW）：**

除咗逐段「✦ AI」按鈕，仲有一個常駐嘅「AI 助手」聊天窗口，可以用一句自然語言指令做批量／跨段修改：

- **邊度開**：校對頁喺右側「詞彙表」面板底部（「掃描詞彙表」／「全部重新生成」掣旁）撳「**✦ AI 助手**」；主頁喺右上角 topbar（「執行」掣旁）都有同一個掣。視窗可拖動、無遮罩、唔會阻擋你操作其他部分，關咗再開會保留之前嘅對話。
- **可以講咩**：直接打字講你想點改就得，例如：
  - 「把所有『晨操』改成『早操』」
  - 「第 3 段個馬名錯咗，應該係『金鎗六十』」
  - 「第 5 段英文嗰句精簡返」

  AI 會先理解你嘅指令，再喺全片掃描搵出所有相關段落，以**預覽卡片**形式列出，等你確認先至真正修改字幕。
- **卡片點用**：
  - 每行預覽都有 checkbox，可以逐行剔選要唔要套用；已批核嘅段落**預設唔剔**並有標記，避免誤觸已審核好嘅內容
  - 卡片底部「**套用後批核**」開關 — 唔開嘅話套用之後批核狀態維持原狀（等你再覆核）；開咗就套用同時直接標為已批核
  - 套用之後每一行都可以隨時「**還原**」返去修改前嘅文字同批核狀態
  - 段落表有變動（例如做咗分割／合併／AI Rerun）之後，舊卡片會變灰失效，撳「**重新掃描**」即可刷新（呢步唔會再問 AI，即時完成）
  - 如果指令會令某啲字被**整段刪走**（改成空白），卡片會有明顯警示行，提醒你留意先套用
- **「＋ 新對話」掣**清空對話重新開始；AI 記得最近三輪指令嘅摘要（例：改完第 3 段之後講「第 3 段再改多次，語氣輕鬆啲」接得住），但太耐之前／太轉彎嘅指涉未必明 — 卡片預覽會俾你睇清楚先套用。
- **限制**：AI 助手只負責修改字幕文字，唔會回答「點解咁譯」之類嘅內容問題（佢見唔到全片內容，勉強答只會作嘢）；某段正喺度 AI Rerun 期間唔可以套用修改；如果某個輸出正喺度渲染緊，AI 助手套用嘅修改**唔會**反映喺嗰次渲染入面（要重新渲染先睇到）。

> AI 助手同其他 AI 修改功能一樣只支援輸出語言檔案；一般 Profile／V6 檔案唔會顯示呢個掣。

**調整段落時間（僅限輸出語言檔案）：**

底部時間軸右上角有縮放控制：「**⌖ 對焦本段**」會放大到當前段落前後（長片必用），「⊡ 全片」返回全片視圖，放大後可左右捲動。調整時間有三種方法：

1. **拖拉**（最直接）— 當前段落（紫色）左右兩邊有把手，揸住直接拖個頭／個尾；相鄰段落嘅邊界會自動跟住郁（唔會出現重疊或斷層）
2. **I / O 快捷鍵** — 播放或定格到想要嘅時刻，按 `I` 設為該段開始、`O` 設為該段結束（時間軸下方亦有 ⤓I／⤓O 按鈕）
3. **直接輸入** — 點時間軸下方嘅 In／Out 時間碼直接輸入（格式 MM:SS.ss，Enter 確認）

每段最短 0.4 秒，系統會自動限制，唔會整爛時間軸。調整時間**唔會**影響已批核狀態。
...
字幕字型而家可以**喺 app 內自己上載**，唔使再手動將字型檔放入伺服器。

- 喺 **校對頁** 或 **主頁** 嘅「字幕設定」面板，撳 **「＋ 新增字型」** → 揀你部機嘅 `.ttf` / `.otf` 字型檔上載（接受 TrueType / OpenType，檔案上限 32MB）。
- 上載後字型即刻出現喺字型下拉選單（分「已上載字型」同「系統字型」兩組）；揀咗即刻反映喺**預覽**同最終**燒入影片**嘅字幕 —— 兩邊用同一個字型檔，唔會走樣。
- 修復咗之前「揀其他字型好似冇反應」嘅問題：以前字型清單係寫死嘅系統字型名，部機冇安裝就全部 fallback 成同一隻字 → 而家清單只列**真正可用**嘅字型，所以切換一定見到分別。
- 揀完字型記得撳「**儲存為預設**」先會套用到燒入輸出。
- 字型檔存放喺 `backend/assets/fonts/`（全域共用）；後端會用 `fonttools` 讀出字型嘅正式 family 名，確保預覽同輸出對齊。
...
每條 entry 有 `原文` / `譯文` 兩個必填欄位，加可選嘅 `譯文別名`（`target_aliases`）同 `近音寫法`（`source_variants`，英文源專用）列表。
...
同一欄多個值用 `;` 分隔。**向後兼容**：舊嘅 2 欄（`source,target`）同 3 欄（`source,target,target_aliases`）CSV 照樣匯入得到；匯出一律出 4 欄。
...
- **校對頁逐段對照**：「詞彙對照」欄位顯示語言標籤 + 觸發詞 + `改前 → 改後 · 術語表名稱`，有改動的段落在段列表顯示 📖 標記，方便快速確認套用結果。
- **多術語表衝突**：以輸入順序為優先（first-wins），不同術語表同一詞目以排前者為準。

### 名詞括號「」（2026-07-07 新增）

如果術語表放嘅係**名詞**（馬名／騎師名），可以喺術語表頁開啟「名詞括號」選項 — 出字幕時該表命中嘅名詞會自動用「」括住（例如 `巴閉佬` → `「巴閉佬」`）。三檔選擇：

| 選項 | 效果 |
|---|---|
| **唔括**（預設） | 維持現狀 — 名詞不加括號（原有嘅統一除括號行為不變） |
| **只中文軌** | 中文字幕軌（粵語口語／書面語／普通話）嘅命中名詞括「巴閉佬」；英文軌不括 |
| **全部軌** | 所有語言軌都括，包括英文軌嘅 `「GOLDEN SIXTY」` |

- 設定位置：術語表頁揀選任何一個表 → 右上「名詞括號」下拉；已開啟嘅表喺左側列表顯示「」標記。
- **只括真實命中**：只有該段字幕真係對中術語表嘅名先會括 — 普通句子入面啱巧出現嘅字眼（例如馬名「關鍵所在」出現喺「這就是關鍵所在」）唔會被誤括。
- 生效時間：新上傳／重新處理／AI Rerun／全部重新生成嘅字幕先會套用；已出咗嘅舊檔唔會自動改變。校對頁「逐項 AI 套用」亦會跟從呢個設定。

### 校對頁術語表掃描與逐項套用（output_lang 檔）

校對頁提供**兩種術語表操作方式**，按情況選用：

**🔍 掃描詞彙表（推薦）**

適合完成轉錄後想確認或補正個別術語，**唔影響批核狀態**：

1. 校對頁左側詞彙表 panel 顯示所有可用術語表，勾選即代表此檔案使用（勾選順序 = 優先次序，數字 badge 標示）。勾選後設定即時儲存。
2. 撳「**🔍 掃描詞彙表**」— 系統毫秒級掃描每條輸出語言軌，對每個術語表條目按雙軌路由規則（中/粵語字幕軌：直接比對字幕字眼；跨語系字幕軌：按原文命中後比對字幕有無用標準譯名）分類結果為「待修正」或「已符合」。
3. 掃描 modal 按語言軌分區顯示結果，每個命中項顯示別名 → 標準名、來源術語表、段號+時間（可撳跳轉影片）、字幕原句（別名黃底 highlight）。
4. 已批核段落的候選項預設**不勾選**（batch_status 唔會被覆寫）；其餘候選預設勾選。
5. 確認後撳「**套用選中 (N)**」— AI 逐項串行替換，每項行內即時顯示 ✓（成功）或 ✗（失敗+原因，可重試）。套用完成後顯示總結 toast。
6. **已批核狀態完全保留** — 此操作唔會改任何段落嘅批核狀態。

**⟳ 全部重新生成（慎用）**

由已快取嘅 ASR base 重新 re-derive 整條字幕鏈（無需重新轉錄），適合術語表大改後一次過重跑：

> ⚠️ **破壞性操作警告**：「全部重新生成」**會覆蓋所有人手編輯、批核狀態全部重設**。系統會彈出確認提示，請仔細考慮後再繼續。重生成時間視片長而定（通常需要數十秒至數分鐘）。

1. 撳「**⟳ 全部重新生成**」。
2. 確認警告提示。
3. 系統重新套用現時詞彙表設定，完成後自動更新段落表。
