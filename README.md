...
- **校對頁一鍵回饋**：喺**校對頁**撳「🔍 掃描詞彙表」，掃描視窗除咗「待修正／已符合」，仲多咗一節「**疑似聽錯**」— 系統用確定性近音比對（**零 AI**）揪出字幕入面同術語表／行話表好接近但唔完全一樣嘅寫法（例如 `Malpenza ≈ MALPENSA`）。逐項有「**＋ 加為近音別名**」掣，撳一下就把嗰個聽錯樣寫入對應詞條嘅別名（英文近音 → `source_variants`、中文譯名近音 → `target_aliases`、行話 → 系統行話表）。撳完之後同樣要「**全部重新生成**」先對成個舊檔生效。
- **系統行話表（管理員專屬）**：賽馬等專業行話（如「殿後」「後上」）由一份全局共用嘅**系統行話表**管理。管理員喺術語表頁會見到「**系統行話表**」入口（一般用戶見唔到），可以為每個行話術語加近音聽錯樣（例如「殿後」加「電流位」）。因為係全局資源，只有管理員可以改；非管理員嘗試儲存會被拒（提示「需要管理員」）。
