...
- **已知 minor（非 blocker）**：轉錄期間 progress shim 報 `profile` kind（`report_from_subtitle_segment` hardcode）→ queue bar 可用但 step-diagram 暫顯 profile 3-step；`/api/files` row 未 echo raw `output_languages`（descriptor `languages` 已有 → frontend 無影響）。
