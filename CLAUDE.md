...
- **Phase 6 hand-off backlog (Tier 3 deferred)**: rate limiting on /login + /api/queue (flask-limiter); password policy (min length, blocklist); /api/files O(N) optimization (per-request job_id lookup is O(jobs × files)); frontend addEventListener leaks (queue-panel.js setInterval never clears); app.py / index.html refactor (3000+ / 5000+ lines); /api/ready endpoint (separate from /api/health); systemd hardening (NoNewPrivileges, PrivateTmp); faster-whisper BatchedInferencePipeline (newer API); pytest real_auth marker (replace _REAL_AUTH_MODULES tuple); failed-login audit log entry.
