...
| POST | `/api/files/<id>/glossary-add-alias` | 一鍵把「疑似聽錯」寫成永久別名（宣告層）：body `{kind: source\|target\|lexicon, variant, canonical, glossary_id?, entry_id?, style?}`；`source`→entry `source_variants`、`target`→entry `target_aliases`（無 `entry_id` 靠 `strip_horse_id(target)==canonical` 反查）、`lexicon`→系統行話表 variants（管理員）。冪等（已存在 no-op）；壞 kind/variant → 400、entry/glossary 唔存在 → 404、無權改（共享表 / 非管理員 lexicon）→ 403 中文訊息、`update_entry` ValueError → 422 |
