...
- **3-way translation ensemble**: K0 baseline (`SYSTEM_PROMPT_FORMAL`) + K2 brevity (`SYSTEM_PROMPT_BREVITY_TC`) + K4 entity-aware rewrite (`_brevity_rewrite_pass` with explicit must-keep entity list); orchestrated in parallel via `ThreadPoolExecutor` (max_workers=2 for K0+K2, sequential K4) at [backend/translation/sentence_pipeline.py](backend/translation/sentence_pipeline.py) `translate_with_a3_ensemble()`
- **Ensemble selector** ([backend/translation/a3_ensemble.py](backend/translation/a3_ensemble.py)): per-segment max(F1 entity recall) + CPS≤9.0 gate; tiebreaker prefers K4 (shortest); length safety falls back to next candidate when winner > 32c, flags `k4_unrescuable` when all 3 too long
- **Hybrid wrap algorithm** (`wrap_hybrid` in [backend/subtitle_wrap.py](backend/subtitle_wrap.py)): soft 14 / hard 16 / max 2 lines / bottom-heavy, **lock-aware 4-pass fallback** (BH range → full hard-cap → any unlocked → forced cut + `lock_violated` flag). New `WrapResult2` dataclass extends WrapResult with `bottom_heavy_violation` + `lock_violated` flags
- **V_R11 lock chain reused**: `_build_full_lock(text, glossary_terms)` wrapper in `sentence_pipeline.py` chains `_build_locked_mask` + translit_runs + glossary + dot_heuristic
- **Entity recall** ([entity_recall.py](backend/translation/entity_recall.py) seed 38 entries + [proxy_entities.py](backend/translation/proxy_entities.py) Mod 1 capitalization NER + translit char run heuristic) → glossary `glossary_updated` socket event auto-extends NAME_INDEX (Mod 2)
- **Translation queue** ([queue.py](backend/translation/queue.py) Mod 8): per-user semaphore=1, global semaphore=4, optional job coalescing — protects against OpenRouter 429 rate limits
- **F/B parity harness** (5-tier): Tier P1 canonical fixtures `wrap_canonical_fixtures.json` (30 cases, 0 prototype divergences) + Tier P2 Python pytest + Tier P3 Playwright headless JS evaluation + Tier P4 pre-commit hook (`scripts/check_wrap_parity.sh` blocks commit if `subtitle_wrap.py` modified without `subtitle-wrap.js`) + Tier P5 DOM tspan check
- **5 validation gates** (G1-G5): G1 unit + G2 integration ([test_a3_pipeline.py](backend/tests/integration/test_a3_pipeline.py)) + G3 regression ([run_regression.py](backend/tests/validation/run_regression.py) with 4 golden corpora + thresholds) + G4 pre-merge + G5 production telemetry (`pipeline_timing` event with `metrics: {M1, M2, F1, L1, source_distribution}`)
- **Profile preset**: new `cityu_hybrid` (16/16/2 hybrid + bottom-heavy=true) + `translation.a3_ensemble: true` flag (default in `prod-default.json`); legacy `broadcast` / `netflix_*` presets unchanged
- **Mod 3 — Pass 4 lock violation surfaced**: renderer appends `'lock-violated'` flag to seg when `wrap_hybrid` Pass 4 fired with locks; proofread UI shows red 🚨 違規 badge + amber ⚠ 斷 (wrap-hardcut) + orange ⏩ 速 (cps-overflow) + indigo K0/K2 source badges
- **Mod 6 — Lazy migration**: `/api/render` POST endpoint detects legacy translations (no `source` field) when active profile has `a3_ensemble=true`, spawns background `_run_a3_migrate_async`, returns 202 + `a3_migration_started` socket event; legacy clients (a3_ensemble disabled) unchanged
- **Frontend** ([font-preview.js](frontend/js/font-preview.js) cityu_hybrid branch routes through `window.SubtitleWrap.wrapHybrid` for multi-line bottom-heavy `<tspan>` rendering; [subtitle-wrap.js](frontend/js/subtitle-wrap.js) JS port with `_lockedTruthy` helper mirroring Python `bool(locked)` semantics)
- **Cost & Runtime**: +$0.025/file, +40s latency vs current K0-only baseline (3-way LLM calls but K0+K2 parallel)
- **Validation evidence**: 45 rounds across 4 corpora — A3v3 winner: F1=83.7%, M2=96.8%, L1=0. See [docs/superpowers/specs/2026-05-02-line-budget-validation-tracker.md](docs/superpowers/specs/2026-05-02-line-budget-validation-tracker.md)
- **Tests**: 30 canonical wrap fixtures + 32 wrap_hybrid pytest + 6 entity_recall + 6 proxy_entities + 3 brevity_rewrite + 3 brevity_prompt + 6 a3_ensemble + 3 translation_queue + 2 sentence_pipeline_a3 + 5 profiles_a3 + 3 renderer_wrap + 3 lazy_migration + 1 G2 integration + 30 Playwright parity = **133+ new tests** all passing
...
- **MT-α2 / V_R10 (2026-05-02)** — Direction A 從 3-direction × 20-round agent-team 研究勝出（vs B 多語 embedding alignment + C LLM polish post-pass）；解 user-reported "Cantonese transliteration name 中切" 問題。
  - **Bug fix**: V_R9 嘅 `min_pos` clamp `max(min_pos, _find_break_point(...))` 會喺 locked region 內反推 break_at 落 locked 位置，等於 lock 失效。新增 `_find_unlocked_anywhere(locked, char_offset, min_pos, max_pos)` helper：forward walk through locked → fallback bounded backward walk（防 orphan）。Wired into `redistribute_to_segments` 兩個位置（min_pos advancement + post-find clamp）。
  - **A.1 — Glossary-driven lock**: `_extend_lock_with_glossary(text, locked, terms)` 鎖每個 active glossary ZH term 嘅 internal positions。Edge boundaries (start/end of term) 仍可斷。`translate_with_sentences` 自動 extract `glossary[*].zh` 傳落 redistribute。
  - **A.2 — Translit char heuristic**: `_TRANSLIT_CHARS` 集合（curated ~150 字 HK/TW/CN broadcast 音譯字符），`_extend_lock_with_translit_runs(text, locked, min_run_len=3)` 鎖任何 ≥3 連續 translit char 嘅 run（允許 `·` 內含）。Catches 雲尼素斯、羅德里哥、卡列拉斯 等冇「·」嘅名。
  - **A.3 (jieba NER) rejected**: 880ms runtime, zero metric gain over A.2 (法蘭高 jieba 切錯做「法蘭」+「高」)。HK transliteration character-level pattern 比 general-purpose Chinese NER 更可靠。
  - 實證 V_R10 (user file dbf9f8a6bda7, 94 segs)：name_split count 2 → **0**（雲尼素斯 / 羅德里哥 / 法蘭高·馬斯坦託諾 / 阿爾瓦羅·卡列拉斯 / 皇家馬德里 全部完整保留）；term_consistency 96.77% → **100%**；runtime 1.5ms → 1.8ms（+0.3ms negligible）；false_positive_locks = 0；no new dep。
  - 4 個新 pytest（translit lock vinicius / dot compound / glossary lock / lock-aware min_pos advancement bug regression）；34/34 sentence_pipeline tests pass。
  - Direction B (sentence-transformers LaBSE/MiniLM embedding alignment) 攞返 +0.011 sem_corr 但 +50-250ms warm runtime + 420MB model dep — 留 opt-in 唔做 default。Direction C (LLM polish post-pass) 9 calls/file ($0.005, 17s sequential) 解到 single_char + 結構性 fixable cases 但 17% 結構性 unfixable — 將來作為 opt-in flag candidate。
  - Validation evidence: docs/superpowers/specs/2026-04-30-validation-tracker.md V_R10 section
