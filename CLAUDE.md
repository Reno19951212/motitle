...
| POST | `/api/files/<id>/glossary-preview` | output_lang only — **機械掃描（純讀零副作用）**：對每條輸出語言軌行同 pipeline 一致嘅 `route_for_output` + 別名 matching；body 可選 `{glossary_ids?}`（缺省用檔案設定；測試用 override，前端永遠唔送 override — 改選先 PATCH 落檔再掃描）；回 `{tracks:[{lang, mode, side, applicable_glossaries, inapplicable_glossaries, items:[{idx, start, kind: fix\|ok\|suspect, alias, canonical, entry_id, glossary_id, glossary, approved}]}], totals}`；**add-only `kind:'suspect'`**（疑似聽錯，確定性零 LLM — en 內容用 `en_correction.judge_candidates`、yue 內容用 `phonetic_correction.match_segments` 生成 fuzzy 命中 `{span, canonical, side: source\|target\|lexicon, source_index, style}`，每軌 cap 40、dedupe + 排除已 fix/ok）；**suspect 掃描係 opt-in**（body `{include_suspects: true}` 先跑 — `judge_candidates` O(cue×詞條) 喺大檔慢，唔可以卡住每次掃描；每軌 fuzzy 掃描封頂 `_SUSPECT_SCAN_CUES=400` cue，`totals` 加 `suspects_scanned`/`suspects_truncated`）；**add-only `kind:'declared'`**（已宣告別名反饋 — 每次掃描都跑 cheap `_declared_for_track`，用同 pipeline 一致嘅 `alias_rewrite` 偵測 base 仲有嘅已宣告變體 `{span, canonical, side}`，提示「重新生成後生效」；去重：target_aliases 已由 `_filter_target_side` 出 `fix` → declared 排除，source_variants 英文側 scan_track 唔 match → declared 係佢唯一反饋；已宣告變體亦排除出 suspect，令「加為別名」閉環唔會再列返做未處理）；被正名保護壓制嘅宣告別名以 `blocked:true, blocked_by` 出（totals 加 `declared_blocked`；banner 只計會生效嘅 `declared`）；**exact 掃描已提速 134×**（`build_name_pattern` `@lru_cache` — 851-cue×1353 詞條 161.2s→1.2s byte-identical，`backend/scripts/scan_bench.py` 驗證）；`totals` 加 `suspect`/`declared`；400 非 output_lang / 含未知 glossary id；零 LLM、零 registry 寫入 |
...
| POST | `/api/files/<id>/glossary-add-alias` | 一鍵把「疑似聽錯」寫成永久別名（宣告層）：body `{kind: source\|target\|lexicon, variant, canonical, glossary_id?, entry_id?, style?}`；`source`→entry `source_variants`、`target`→entry `target_aliases`（無 `entry_id` 靠 `strip_horse_id(target)==canonical` 反查）、`lexicon`→系統行話表 variants（管理員）。冪等（已存在 no-op）；反查嚴格 `strip_horse_id` 唔中會退一步用鬆規則剝任何尾括號（同 phonetic `build_index` 一致）；成功 response 帶 add-only `warnings`（§4.2b lint，非阻斷）；壞 kind/variant → 400、entry/glossary 唔存在 → 404、無權改（共享表 / 非管理員 lexicon）→ 403 中文訊息、`update_entry` ValueError → 422 |
| POST | `/api/glossaries/alias-lint` | §4.2b 別名警告安全網（pure lint、零寫入）：body `{variant, kind: source\|target\|lexicon}` → `{warnings:[中文提示…]}`（太短唔會生效 / 常用英文詞誤中風險 — 中文冇常用詞表，只做長度警告）；三個前端別名入口用嚟做非阻斷 toast；壞參數 → 400 |
| GET | `/api/lexicons/<style>` | 系統行話表（phonetic lexicon）view — 登入即可讀；回 `{style, terms:[{term, variants}]}`（dual-shape 正規化：bare string → `variants:[]`）；未知 style → 404 |
| PUT | `/api/lexicons/<style>` | 系統行話表 bulk 覆寫 — **管理員專屬**（全局共用資源）；body `{terms:[{term, variants}]}`；`variants` 空 → 寫返 bare string（精簡）；未知 style → 404、壞 body → 400 |
...
- 驗證：發音編碼索引 REJECT（candidate 爆 67-84×、200 上限靜默截走 99%）；宣告機制 gating GATE1-3 PASS（[tracker](docs/superpowers/specs/2026-07-14-glossary-alias-validation-tracker.md)、[design](docs/superpowers/specs/2026-07-14-glossary-fuzzy-alias-design.md)）。
- **閉環 UI 已通（Plan B, 2026-07-15）**：三個「填得落」入口令聽錯一㩒變永久別名 —— ①**術語表頁**（`Glossary.html`）詞條詳情加「近音寫法 (原文別名)」`source_variants` chip 列 + 表格「近音 N」badge + 搜尋；②**系統行話表 modal**（2026-07-16 起全員可讀 — 非管理員開到只讀視圖，修改控件 `isAdmin` gate；GET `/api/lexicons/<style>` 登入即可、PUT 管理員專屬，`backend/lexicon_manager.py` 原子讀寫 + dual-shape 保留 + per-style lock）；③**校對頁掃描 modal**（`glossary-review.js`）加「疑似聽錯」分節（來自 `glossary-preview` 的 `kind:'suspect'`，零 LLM；**opt-in — 「🔎 搵疑似聽錯」掣先觸發**，令默認 exact 掃描保持快）+ 一鍵「＋ 加為近音別名」→ `POST /api/files/<id>/glossary-add-alias`（authz per kind：source/target = `can_edit`、lexicon = admin）。粵語 candidate 冇 `entry_id` → 靠 `strip_horse_id(target)==canonical` 反查，`supplement` canonical → 路由去 lexicon variants。round-trip 驗證 `backend/scripts/aliasui_verify.py`（GATE1 en suspect 生成 + GATE2 lexicon 寫入）。**加咗別名之後要「全部重新生成」(glossary-reapply) / AI Rerun 先對舊檔生效。**
- **校對頁詞彙表面板同步升級（2026-07-15）**：output_lang 校對頁自己嗰個詞彙表面板（同 `Glossary.html` 分開嘅第二套 code）之前只得原文/譯文兩欄 inline 編輯、冇搜尋。而家加咗 **①條目搜尋**（`#glEntrySearch`，比對原文/譯文/`source_variants`/`target_aliases`）**②條目編輯 modal `#geOverlay`**（撳 ✎ / +新增 共用；原文/譯文 + 近音寫法 `source_variants` chip + 別名 `target_aliases` chip，chip 用 `prompt()` 加、批次保存，PATCH/POST `{source,target,source_variants,target_aliases}`）**③每行「·近N」badge**。純前端（`proofread.html`，取代 cramped inline 編輯），後端零改動。**gotcha：modal DOM 喺 body 尾，wiring 要 `DOMContentLoaded` 後先 attach（`_geWire`）。** E2E `frontend/tests/test_proofread_glossary_editor.py`（11/11，真 Chrome）。設計/計劃：`docs/superpowers/{specs,plans}/2026-07-15-proofread-glossary-editor-*`。
- **Plan C（自動學：AI 確認過嘅糾正自動寫入待確認別名列表）仍待做** — 粵語 judge 改動冇 `entry_id`/`glossary_id`，attribution 不對稱，需獨立處理。

### AI 助手聊天窗口（AI Chat Window, NEW 2026-07-14）

- **主頁 + 校對頁**都有「✦ AI 助手」浮動可拖非阻擋窗（`frontend/js/ai-chat.js`，find-replace 同款 pattern，body-mounted z-index 2600，Esc chain 插喺 FindReplace 之前，close/reopen 對話保留，雙頁 `window.AIChatPage` adapter）。用自然語言落字幕修改指令：批量取代（零 LLM apply）＋指定段落 AI 改寫（兩段式：現有 `/ai-edit` 生成 → 卡片預覽實際文字 → 先套用）。視窗頭右上有「＋ 新對話」掣（`#acNew`，喺「✕」關閉掣之前）：清空對話 + 記憶（`turns=[]`／`turnSummaries=[]`）、bump `turnSeq` 令舊 in-flight 回應作廢、插「— 新對話 —」分隔線、焦點返 input；`boundFileId` 唔郁（仍然係同一檔案）。
- **每 turn 恰好 1 個 LLM call**（`_make_ollama_llm_call`，Beta-aware）；transcript 永不入 prompt；目標段 server 機械掃描；多輪 context = 前端機械摘要 **rolling 3 輪**（`turnSummaries[]`，最新喺前，每個 ≤120 字，`composeMemory()` 組成 `上一輪：<最新>；前一輪：<較早>；再前一輪：<最早>`，單輪 degenerate 成 `上一輪：<摘要>`；payload key 一律 `"上一輪"` 唔可以改名 — tracker addendum 2026-07-15 實證改名會 orphan system prompt 嘅 follow-up anchor）；server 只做整條 string `[:300]` clamp，唔重組；間接指涉（例如 ordinal「第一樣嘢」／間接措辭 follow-up）跨輪 recall 未達 6/8 bar，唔係設計保證 — 一律靠人手確認預覽兜底。全部係本地 35b 退化防線。
...
- 實證：研究 clip 34/36=94.4% 修復（書面語錯名殘留 18/36→0/36）；4-clip gating 驗證 PASS（tracker: [2026-06-13-phonetic-correction-validation-tracker.md](docs/superpowers/specs/2026-06-13-phonetic-correction-validation-tracker.md)；研究全卷: [2026-06-13-lang-quality-research/](docs/superpowers/specs/2026-06-13-lang-quality-research/)）。已知限制：~~AI Rerun 重做嘅 cue 唔過糾錯（P2）~~（**已修 2026-07-15**：AI Rerun + 全部重新生成都補跑 base 糾錯層）。新依賴 `ToJyutping`。
