...
- **Cross-FAMILY outputs use the bound-base single-pass derive**: transcribe the content audio ONCE → derive each output 1:1 (no index-merge) via `output_lang_router.route_output()` + `crosslang_mt.translate_segments()` (injected `llm_call`, per-cue 1:1). **Same-family same-dialect** = Whisper-direct (best segmentation/quality); **書面語 (zh)** = V6 formal refiner; then OpenCC 繁/簡 (`script`).
