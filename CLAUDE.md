...
### v6 — VAD + Dual-ASR + Refiner LLM (in progress on `feat/frontend-redesign`)
- 架構上由 v5 嘅「garbage-in / LLM-cleans-up」模式轉向「VAD 從源頭過濾 + qwen3 為 content authority + mlx 純時間軸參考 + simplified refiner」。Qwen3-ASR 負責睇清楚講咗咩，mlx-whisper 只負責幾點幾秒，refiner 唔再需要判斷幻覺因為輸入已經乾淨。
- **Stage 0 — Silero VAD pre-segmentation** ([backend/stages/v6/vad_stage.py](backend/stages/v6/vad_stage.py)): `silero-vad 6.2.1` 將全段 audio 切成多個語音 region（含 padding + merge_gap），輸出 `[(start_sec, end_sec)]` list 入 `StageContext`。靜音片段永遠唔入 ASR，從根源消滅 Whisper 頭部 hallucination cluster。
- **Stage 1A — Qwen3-ASR per-region** ([backend/stages/v6/qwen3_asr_stage.py](backend/stages/v6/qwen3_asr_stage.py)): `mlx_qwen3_asr v0.3.5` 逐個 VAD region 獨立轉寫。每個 region 傳入 `context` 字段（可 per-pipeline 預設 + per-file override），支援喺 prompt 交代廣播話題、人名、地名。輸出純文字，**唔帶時間戳**，作為 content authority。
- **Stage 1B — mlx-whisper full audio** ([backend/stages/v6/mlx_whisper_stage.py](backend/stages/v6/mlx_whisper_stage.py)): 跑完整 audio（唔切 VAD region），抽取 word-level timestamps 做時間軸 grid。Text 只 persist 做審計（`mlx_text` field），**唔作為字幕輸出**。兩條 Stage 1 路徑係 sequential initial release，parallel 優化留後。
- **Stage 2 — TimeAnchoredMergeStage** ([backend/stages/v6/merge_stage.py](backend/stages/v6/merge_stage.py)): 三步 merge — (a) 將 qwen3 char-spans 同 mlx word boundaries 對齊，決定每個 qwen3 輸出 segment 嘅 start/end 時間；(b) VAD region 邊界唔會被跨越（硬性 fence）；(c) fragment merge — qwen3 per-region 輸出偶爾係前後段同一句嘅碎片，merge 至字數達標或遇到句末標點。輸出係帶完整 `{start, end, text}` 嘅 canonical segment list。
- **Stage 3 — Refiner LLM** ([backend/stages/v6/refiner_stage.py](backend/stages/v6/refiner_stage.py)): `qwen3.5:35b-a3b-mlx-bf16` via Ollama，simplified prompt 策略（keep-only，冇 `[HALLUC]` tag 機制 — 輸入已無幻覺）。`_collapse_drops` safety net：Refiner 空輸出或 drop segment 時 fall back 到 Stage 2 原文，確保唔會 silent data loss。支援 per-file prompt override（`refiners.zh` key，沿用 v3.18 機制）。JSON unwrap 自動處理 Ollama 偶爾包裹 `{"text": "..."}` 返回嘅情況；short-input bypass（<5 chars）直接 pass-through 唔 call LLM。`max_tokens=300` hard cap。
- **Stage 4 translator + Stage 5 persist** — 沿用 v5-A2 路徑（`TranslatorStage` + `_persist_by_lang`）；v6 唔改這兩個 stage，out-of-scope for initial release。
- **Pipeline dispatch** ([backend/pipeline_runner.py](backend/pipeline_runner.py)): 新 `pipeline_type: "v6_vad_dual_asr"` enum field（**唔係 version 字段**），`_run_v6()` 分支按此 dispatch。`pipeline_type` 缺席時 fallback 到 `version` 嘅現有邏輯，v4/v5 路徑完全不變。
- **2 個 v6 pipeline JSON configs** 連同 repo 一齊 ship：[backend/config/pipelines/v6_cantonese_racing.json](backend/config/pipelines/v6_cantonese_racing.json)（賽馬廣播，ZH，qwen3 context 預設「香港賽馬新聞」）+ [backend/config/pipelines/v6_english_newscast.json](backend/config/pipelines/v6_english_newscast.json)（Winning Factor EN newscast）。
- **前端整合**：
  - `Pipelines.tsx` refiner prompt panel — v6 pipeline 以 load-by-ID mode 顯示，唔 expose 所有欄位，只顯示 `qwen3_context` + refiner prompt override textarea。
  - `PromptOverridesDrawer` 新增 `qwen3_context` 同 `refiners.zh` 兩個欄位（per-file override，沿用 v3.18 三層 fallthrough：file > pipeline default > None），連帶 `POST /api/files/<fid>/pipeline_overrides` 已有路由直接寫入。
- **5 個設計決策確認入 spec**（由 user 逐一 confirm）：(a) refiner keep-only + `_collapse_drops` safety net；(b) qwen3 context per-pipeline default + per-file override；(c) dispatch 用 `pipeline_type` enum 唔用 `version`；(d) mlx text persist for audit，time-grid only downstream；(e) Stage 1A+1B sequential initial release。
- **17 commits 交付**（T1-T13 主線 + 4 個 post-implementation fix）：T1 VAD stage + silero dep、T2 Qwen3-ASR stage + subprocess bridge、T3 mlx-whisper stage、T4 TimeAnchoredMergeStage + alignment algo、T5 RefinerStage v6 + JSON unwrap、T6 pipeline_runner `_run_v6` dispatch、T7 pipeline JSON configs + StageContext field、T8-T9 frontend Pipelines/PromptOverridesDrawer、T10 tests `test_v6_stages.py`、T11 tests `test_v6_runner.py`、T12 tests `test_v6_refiner_json_unwrap.py`、T13 tests `test_v6_pipeline_config.py`；fix1 `parents[3]` REPO_ROOT path、fix2 JSON wrap Option C、fix3 refiner override nested key、fix4 Path B operator-validation.
- **Operator-validated** on 賽馬 4-min clip（粵語廣播）+ 14-min vlog（英語新聞），詳見 [docs/superpowers/validation/v6-validation.md](docs/superpowers/validation/v6-validation.md)。
- **已知 limitation（spec 確認留後）**：Stage 1.5 mlx pre-cleanup deferred — 每約 10 分鐘 audio 出現 1 個 mlx broken-time mega-segment，需另立 T 處理；per-segment entity normalization deferred — 偶爾同名人物喺唔同 region 嘅轉寫略有不一致。
- **Tests**: ~94 個新 pytest cases — [backend/tests/test_v6_stages.py](backend/tests/test_v6_stages.py)（Stage 0/1A/1B/2 + `StageContext` field，55 cases）+ [backend/tests/test_v6_runner.py](backend/tests/test_v6_runner.py)（`_run_v6` dispatch + override resolution，18 cases）+ [backend/tests/test_v6_refiner_json_unwrap.py](backend/tests/test_v6_refiner_json_unwrap.py)（JSON unwrap + short-input bypass + `max_tokens=300`，14 cases）+ [backend/tests/test_v6_pipeline_config.py](backend/tests/test_v6_pipeline_config.py)（pipeline JSON validity，7 cases）；另 ~5 個 vitest cases for frontend PromptOverridesDrawer v6 fields。
- **Spec / Plan / Validation**: [design](docs/superpowers/specs/2026-05-21-v6-vad-dual-asr-refiner-design.md) / [plan](docs/superpowers/plans/2026-05-21-v6-vad-dual-asr-refiner-plan.md) / [validation](docs/superpowers/validation/v6-validation.md)
...
- **R6 — Pipeline validator warnings channel** ([backend/pipeline_schema_v5.py](backend/pipeline_schema_v5.py)): `validate_v5_pipeline` returns `(errors, warnings)` tuple. Warns on: source_lang not in target_languages; target language present but `translators.<tgt>` missing. Surfaced in `POST /api/pipelines` 201 + 400 + `PATCH /api/pipelines/<id>` 200 response body as `warnings: [...]`. Non-blocking advisory — provides human-readable detail alongside hard errors.
- **Tests**: **32 new pytest cases** in [backend/tests/test_v5_bloat_hardening.py](backend/tests/test_v5_bloat_hardening.py) — engine cap assertions, meta-prefix parametrized, verifier R1 four-quadrant table + 3.0s boundary, flag computation, validator tuple shape, validator warning rules, end-to-end runaway-LLM smoke. Full backend suite: 912 pass + 21 skip + 14 known baseline failures (unchanged).
- **Out of A4 scope** (Phase 5+): source-lang auto-detection from audio; re-segmenting secondary's long-window output; v5 branch in `PipelineManager.update_if_owned` (TODO placed in `routes/pipelines.py`).
