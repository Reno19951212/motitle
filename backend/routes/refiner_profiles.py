...
Endpoints (all login_required):
  ...
  GET    /api/refiner_profiles               List visible (owner OR admin OR shared)
  POST   /api/refiner_profiles               Create (current user is owner)
  GET    /api/refiner_profiles/<pid>         View single (admin OR owner OR shared)
  PATCH  /api/refiner_profiles/<pid>         Update (admin OR owner only)
  DELETE /api/refiner_profiles/<pid>         Delete (admin OR owner only)

Errors:

  400 invalid payload (validation errors — missing lang/style/etc.)
...
Successor to v4 ``/api/mt_profiles`` (which now serves Deprecation/Link/Sunset
headers pointing here). Narrowed to same-lingual polish — `lang` is both input
and output. For cross-lingual conversion use TranslatorProfile.
"""
