...
Endpoints (all login_required):
  ...
  GET    /api/translator_profiles               List visible (owner OR admin OR shared)
  POST   /api/translator_profiles               Create (current user is owner)
  GET    /api/translator_profiles/<pid>         View single (admin OR owner OR shared)
  PATCH  /api/translator_profiles/<pid>         Update (admin OR owner only)
  DELETE /api/translator_profiles/<pid>         Delete (admin OR owner only)

Errors:

  400 invalid payload (validation errors, including source_lang == target_lang)
...
This is a NEW v5 entity — no v4 deprecation predecessor. Cross-lingual translation
profile (source_lang ≠ target_lang); same-language register polish belongs in the
RefinerProfile blueprint instead.
"""
