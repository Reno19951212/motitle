...
Same-lingual polish (broadcast register, glossary, disfluency removal).

Narrows v4 MT profile semantic: NO translation, NO target_language field
(`lang` is both input and output). For cross-lingual conversion, use
TranslatorProfile instead.
...
`backend/config/prompt_templates_v5/refiner/`).
...
class RefinerProfileManager:
    """CRUD + ownership for RefinerProfile entities.
