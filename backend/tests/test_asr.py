...
def test_whisper_schema_does_not_include_word_timestamps():
    """word_timestamps was dropped in v4 A1 Q7-b; schema must not expose it."""
    from asr.whisper_engine import WhisperEngine
    engine = WhisperEngine({"engine": "whisper", "model_size": "tiny"})
    schema = engine.get_params_schema()
    assert "word_timestamps" not in schema["params"]
