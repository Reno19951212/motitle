...
def test_get_translations(client_with_file):
    ...
    # v5-A2 T7 (inverted) — v4 shape is the default; v5 by_lang shape is opt-in
    # via ?shape=v5 (see test_v5_a2_normalize.py).
...
def test_approve_single(client_with_file):
    ...
    assert resp.status_code == 200
    # v5-A2 T7 (inverted) — default v4 shape exposes top-level status, which approve POST writes.
...
def test_get_translations_normalizes_legacy_prefix_on_read(tmp_path):
    ...
    try:
        with app.test_client() as c:
            # v5-A2 T7 (inverted) — v4 shape is the default response.
