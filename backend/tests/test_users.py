...
def test_create_user_returns_id(db_path):
    ...
    init_db(db_path)
    uid = create_user(db_path, username="alice", password="TestPass1!", is_admin=False)
...
def test_create_duplicate_username_fails(db_path):
    ...
    with pytest.raises(ValueError, match="exists"):
        create_user(db_path, username="alice", password="TestPass2!")


def test_get_user_by_username(db_path):
    from auth.users import init_db, create_user, get_user_by_username
    init_db(db_path)
    create_user(db_path, username="alice", password="TestPass1!", is_admin=True)
...
def test_get_user_by_id(db_path):
    ...
    init_db(db_path)
    uid = create_user(db_path, username="bob", password="TestPass1!")
...
def test_verify_credentials_success(db_path):
    ...
    init_db(db_path)
    create_user(db_path, username="alice", password="TestPass1!")
    u = verify_credentials(db_path, "alice", "TestPass1!")
