...
Stores ASR (transcribe) engine config as standalone profiles. Replaces the v4
``asr_profiles.py`` semantic with explicit v5 patterns:

  - ``shared: true`` field grants read access (not v4's ``user_id: None`` sentinel)
  - Immutable id / user_id / created_at preserved on update
  - ``updated_at`` audit field

Supports engines:

  - ``whisper`` — faster-whisper / openai-whisper
  - ``mlx-whisper`` — Apple Silicon MLX backend
  - ``qwen3-asr`` — new in v5 (Alibaba Qwen3-ASR)
...
This mirrors the v5 LLMProfile pattern (``backend/llm_profiles.py``) — when
this pattern needs to evolve, update both modules together.
"""
...
def validate_transcribe_profile(data: Any) -> list:
    ...
    # initial_prompt is optional; if present, must be string within cap.
    ip = data.get("initial_prompt", "")
    if ip:
        if not isinstance(ip, str):
            errors.append("initial_prompt must be string")
        elif len(ip) > MAX_INITIAL_PROMPT_CHARS:
            errors.append(f"initial_prompt max {MAX_INITIAL_PROMPT_CHARS} chars")
    # model_size is optional; if present, must be non-empty string.
    ms = data.get("model_size")
    if ms is not None and (not isinstance(ms, str) or not ms.strip()):
        errors.append("model_size must be non-empty string when provided")
    return errors


class TranscribeProfileManager:
    """CRUD + ownership for TranscribeProfile entities.
